    ...
    private void doubleCapacityIfFull() {
      if (_count >= _array.Length) {
        ...
        if (_front <= front) {
          ...
          Array.Copy(_array, (int)_front, newArray, 0, (int)_count);
        } else {
          //values do wrap around, we need to use 2 copies
          uint backOffset = (uint)_array.Length - _front;
          Array.Copy(_array, (int)_front, newArray, 0, (int)backOffset);
          Array.Copy(_array, 0, newArray, (int)backOffset, (int)(_count - backOffset));
        }

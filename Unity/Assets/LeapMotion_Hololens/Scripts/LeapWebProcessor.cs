...
public class LeapWebProcessor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        IsConnected = false;
        foreach(Transform child in GameObject.Find("HandsManager").transform)
        {
    ...
    private Hand creatHand(long frameId, object hand, object pointables_object)
    {
        ...
        Hand result = new Hand(frameId, (int)Handid, (float)confidence, (float)grabStrength, 0, (float)pinchStrength, 0, (float)s, isLeft, (float)timeVisible, arm, fingerlist, palmPosition, stabilizedPalmPosition, palmVelocity, palmNormal, LeapQuaternion.Identity, direction, wrist);

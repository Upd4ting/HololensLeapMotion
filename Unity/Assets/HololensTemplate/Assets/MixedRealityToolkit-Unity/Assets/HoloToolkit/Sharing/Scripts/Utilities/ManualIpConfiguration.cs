...
namespace HoloToolkit.Sharing.Utilities
{
    ...
    public class ManualIpConfiguration : MonoBehaviour
    {
        ...
        public void ConnectToSharingService()
        {
            ...
            GameObject.Find("Sharing").GetComponent<AutoJoinSessionAndRoom>().StartConnection();
            GameObject.Find("Sharing").GetComponent<SharingWorldAnchorManager>().StartManager();
#if !UNITY_EDITOR
            if(IsTracking)
                GameObject.Find("HandsManager").GetComponent<LeapWebProcessor>().InitSocket(ipAddress.text);
#endif
            CheckConnection();
        }
